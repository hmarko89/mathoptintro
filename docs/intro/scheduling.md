...
The processing time of a job $j$, $p_{ij}$ (or simply $p_j$ in the single machine case), is always given.
...
In case of *unrelated parallel machines*, $\alpha = \operatorname{R}$.
...
In case of *open shop*, $\alpha=\operatorname{O}$, the order in which the operations are executed is immaterial.
In case of *flow shop*, $\alpha=\operatorname{F}$
...
Flow shop                   | F        | Shop problem. 
Job shop                    | J        | Shop problem. 
...
The *lateness* of a job is $L^S_j = C^S_j - d_j$, while its *tardiness* is $T^S_j = \max\{0,L^S_j\}$.

The *flow time* of a job is $F_j = C_j - r_j$.

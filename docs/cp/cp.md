...
??? example "Sudoku – How to solve it on paper?"
    ...
    If you’re lucky, you’ll quickly find out whether you made a bad choice: some contradiction will appear — for example, a cell ends up with no valid values left according to the rules. In that case, you can safely remove the number you just tried from the cell’s possible candidates.
    If you do not get lost in the bookkeeping, this "trial and error with logic" strategy will guarantee the solution to the puzzle.
